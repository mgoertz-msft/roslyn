...
namespace Microsoft.CodeAnalysis.Xml.Syntax.InternalSyntax
{
    ...
    internal class LanguageParser : SyntaxParser
    {
        ...
        internal TNode ParseWithStackGuard<TNode>(Func<TNode> parseFunc, Func<TNode> createEmptyNodeFunc) where TNode : XmlSyntaxNode
        {
        ...
        private TNode CreateForGlobalFailure<TNode>(int position, TNode node) where TNode : XmlSyntaxNode
        {
        ...
        {
            public SyntaxListBuilder<XmlSyntaxNode> Elements;

            public XmlBodyBuilder(SyntaxListPool pool)
            {
                Elements = pool.Allocate<XmlSyntaxNode>();
            }
        ...
        internal XmlBodySyntax ParseXml()
        {
            return ParseWithStackGuard(
                ParseXmlCore,
                () => SyntaxFactory.XmlBody(
                    new SyntaxList<XmlSyntaxNode>(),
                    SyntaxFactory.Token(SyntaxKind.EndOfFileToken)));
        }

        internal XmlBodySyntax ParseXmlCore()
        {
            XmlBodySyntax result = null;
            ...
            var body = new XmlBodyBuilder(_pool);
            try
            {
                ParseXmlElementSyntaxBody(ref body, EndOfFile, out missingTagCount);
                ValidateDepth();
                Validate.Retail(missingTagCount == 0);
                var eof = this.EatToken(SyntaxKind.EndOfFileToken);
                result = _syntaxFactory.XmlBody(body.Elements, eof);
            }
            ...
            //if (dummy.Nodes != null)
            //{
            //    foreach (XmlNodeSyntax node in dummy.Nodes)
            //    {
        ...
        private void ParseXmlElementSyntaxBody(ref XmlBodyBuilder body, TokenSet followers, out int missingTagCount)
        {
            missingTagCount = 0;
            ...
            while (true)
            {
                switch (_token.Kind)
                {
                    case SyntaxKind.CharacterData:
                        ...
                        var cdata = _syntaxFactory.XmlCDataSection(SyntaxFactory.Token(SyntaxKind.XmlCDataStartToken), builder.ToList(), SyntaxFactory.Token(SyntaxKind.XmlCDataEndToken));
                    ...
                    case SyntaxKind.LiteralContentString:
                        ...
                        var content = SyntaxFactory.XmlTextLiteral(null, _token.Text, _token.Text, null);
                    ...
                    case SyntaxKind.StartOfTag:
                        ...
                        XmlNodeSyntax childElement = ParseXmlElement(followersOrXmlContent, out missingTagCount);
                        DecreaseDepth();
        ...
        //private static void FixWhitespace(XmlElementSyntax element)
        //{
        ...
        private XmlNodeSyntax ParseXmlElement(TokenSet followers, out int missingTagCount)
        {
            ...
            {
                name = _syntaxFactory.XmlName(null, null);
            ...
            try
            {
                ...
                if (this.CurrentToken.Kind == SyntaxKind.GreaterThanToken)
                {
                    ...
                    {
                        IncreaseAndCheckDepth();
                        ParseXmlElementSyntaxBody(ref body, followers | SyntaxKind.StartOfClosingTag, out missingTagCount);
                        DecreaseDepth();
                        ...
                        if (lessThanSlash.IsMissing)
                        {
                            ...
                            lessThanSlash = this.WithXmlParseError(lessThanSlash, ErrorCode.ClosingTagMismatch, name.ToString());
                        ...
                        else
                        {
                            ...
                            if (lessThanSlash.GetTrailingTriviaWidth() > 0 || endName.GetLeadingTriviaWidth() > 0)
                            {
                                ...
                                endName = this.WithXmlParseError(endName, ErrorCode.IllegalWhitespaceInIdentifier);
                            }

                            if (!endName.IsMissing && !MatchingXmlNames(name, endName))
                            {
                                endName = this.WithXmlParseError(endName, ErrorCode.ClosingTagMismatch, endName.ToString(), name.ToString());
                            }
                        ...
                        //this.ResetMode(saveMode);
                        return SyntaxFactory.XmlElement(startTag, body.Elements.ToList(), endTag);
                    }
                    finally
                    {
                        body.Free(_pool);
                ...
                else
                {
                    ...
                    {
                        slashGreater = this.WithXmlParseError(slashGreater, ErrorCode.ClosingTagMismatch, name.ToString());
                    }
        ...
        //private void ParseXmlStartTag(XmlElementSyntax element, int startPos, int startLine, out int endPos, out bool expectEndTag)
        //{
        //    if (!_scanner.WasWhitespaceSkipped)
        //    {
        ...
        //    {
        //        element.Attributes = new List<XmlAttributeSyntax>();
        ...
        //            element.Attributes.Add(ParseXmlAttributeSyntax());
        //        }
        //    }
        ...
        //    //{
        //    //    XmlAttributeSyntax attribute = element.Attributes?.LastOrDefault();
        ...
        //private void ParseXmlEndTag(XmlElementSyntax element, out int endPos, out int missingTagCount)
        //{
        //    missingTagCount = 0;
        //    XmlElementSyntax parent = element.Parent;
        ...
        //private XmlAttributeSyntax ParseXmlAttributeSyntax()
        //{
        //    XmlAttributeSyntax attribute = new XmlAttributeSyntax();
        //    int startPos = _scanner.StartPos;
        ...
        private bool IsXmlNodeStartOrStop()
        {
            switch (this.CurrentToken.Kind)
            {
                ...
                case SyntaxKind.Comment:             //case SyntaxKind.XmlCommentStartToken:
                case SyntaxKind.XmlCDataStartToken: //case SyntaxKind.XmlProcessingInstructionStartToken:
                case SyntaxKind.ProcessingInstruction:
        ...
        private XmlNameSyntax ParseXmlName()
        {
            ...
            return _syntaxFactory.XmlName(prefix, id);
        }
        ...
        private void ParseXmlAttributes(ref XmlNameSyntax elementName, SyntaxListBuilder<XmlAttributeSyntax> attrs)
        {
            ...
            while (true)
            {
                if (this.CurrentToken.Kind == SyntaxKind.IdentifierToken)
                {
                    ...
                    {
                        attr = this.WithXmlParseError(attr, ErrorCode.AttributeAlreadyDefined, attrName);
                    }
                    else
                    {
        ...
        private XmlAttributeSyntax ParseXmlAttribute(XmlNameSyntax elementName)
        {
            ...
            if (attrName.GetLeadingTriviaWidth() == 0)
            {
                ...
                attrName = this.WithXmlParseError(attrName, ErrorCode.ExpectedWhitespace);
            }

            var equals = this.EatToken(SyntaxKind.EqualsToken, false);
            if (equals.IsMissing)
            {
                equals = this.WithXmlParseError(equals, ErrorCode.ExpectedDifferentToken, "=");
        ...
        private SyntaxToken ParseXmlAttributeStartQuote()
        {
            ...
            {
                startQuote = this.WithXmlParseError(startQuote, ErrorCode.ExpectedDifferentToken, "\"");
            }
            return startQuote;
        }
        ...
        private SyntaxToken ParseXmlAttributeEndQuote(SyntaxKind quoteKind)
        {
            ...
            {
                endQuote = this.WithXmlParseError(endQuote, ErrorCode.ExpectedDifferentToken, "\"");
            }
            return endQuote;
        }
        ...
        private SyntaxToken SkipNonAsciiQuotationMark()
        {
            ...
            quote = this.WithXmlParseError(quote, ErrorCode.ExpectedDifferentToken, "\"");
            return quote;
        }
            ...
            ) where T : XmlSyntaxNode
        {
            ...
            try
            {
                ...
                if (!badTokens.IsNull && badTokens.Count > 0)
                {
                    ...
                    {
                        list[list.Count - 1] = AddTrailingSkippedSyntax((XmlSyntaxNode)list[list.Count - 1], badTokens.ToListNode());
                    }
        ...
        private TNode WithXmlParseError<TNode>(TNode node, ErrorCode code) where TNode : XmlSyntaxNode
        {
            return WithAdditionalDiagnostics(node, new SyntaxDiagnosticInfo(0, node.Width, code));
        }

        private TNode WithXmlParseError<TNode>(TNode node, ErrorCode code, params string[] args) where TNode : XmlSyntaxNode
        {
            return WithAdditionalDiagnostics(node, new SyntaxDiagnosticInfo(0, node.Width, code, args));
        }

        private SyntaxToken WithXmlParseError(SyntaxToken node, ErrorCode code, params string[] args)
        {
            return WithAdditionalDiagnostics(node, new SyntaxDiagnosticInfo(0, node.Width, code, args));
        }
