...
namespace Microsoft.CodeAnalysis.Xml.Syntax.InternalSyntax
{
    ...
    /// <summary>
    /// Scans Xml source text for tokens.
    /// </summary>
    internal class Lexer : AbstractLexer
    {
        ...
        ///<summary>The XML parse options.</summary>
        private XmlParseOptions _options;
        ...
        internal Lexer(SourceText text, XmlParseOptions options) : base(text)
        {
            _source = text;
            _options = options;
        ...
        internal SyntaxToken Create(ref TokenInfo info, SyntaxListBuilder leading, SyntaxListBuilder trailing, SyntaxDiagnosticInfo[] errors)
        {
            ...
            switch (info.Kind)
            {
                ...
                case SyntaxKind.XmlTextLiteralNewLineToken:
                    token = SyntaxFactory.XmlTextNewLine(leadingNode, info.Text, info.Text, trailingNode);
                    break;
                case SyntaxKind.XmlTextLiteralToken:
                    token = SyntaxFactory.XmlTextLiteral(leadingNode, info.Text, info.Text, trailingNode);
                    break;
        ...
        private void AddTrivia(XmlSyntaxNode trivia, ref SyntaxListBuilder list)
        {
            if (this.HasErrors)
            {
        ...
        private XmlSyntaxNode ScanEndOfLine()
        {
            char ch;
