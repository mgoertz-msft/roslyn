...
namespace Microsoft.CodeAnalysis.Xml.Syntax.InternalSyntax
{
    ...
    {
        protected readonly Lexer _scanner;
        ...
        protected void ReInitialize()
        {
            ...
            if (this.IsIncremental)
            {
                _firstBlender = new Blender(this._scanner, null, null);
            }
        }
        ...
        protected Xml.XmlSyntaxNode CurrentNode
        {
            get
            {
        ...
        private void AddNewToken()
        {
            ...
            {
                this.AddLexedToken(this._scanner.Lex());
            }
        }
        ...
        private static ErrorCode GetExpectedTokenErrorCode(SyntaxKind expected, SyntaxKind actual)
        {
            switch (expected)
            {
                ...
                default:
                    return ErrorCode.ExpectedDifferentToken;
            }
        }
        ...
        protected void GetDiagnosticSpanForMissingToken(out int offset, out int width)
        {
            ...
            if (trivia != null)
            {
                SyntaxList<XmlSyntaxNode> triviaList = new SyntaxList<XmlSyntaxNode>(trivia);
        ...
        protected TNode AddError<TNode>(TNode node, int offset, int length, ErrorCode code, params object[] args) where TNode : XmlSyntaxNode
        {
            return WithAdditionalDiagnostics(node, MakeError(offset, length, code, args));
        }

        protected TNode AddError<TNode>(TNode node, XmlSyntaxNode location, ErrorCode code, params object[] args) where TNode : XmlSyntaxNode
        {
        ...
        protected TNode AddErrorToFirstToken<TNode>(TNode node, ErrorCode code) where TNode : XmlSyntaxNode
        {
            var firstToken = node.GetFirstToken();
            return WithAdditionalDiagnostics(node, MakeError(firstToken.GetLeadingTriviaWidth(), firstToken.Width, code));
        }

        protected TNode AddErrorToFirstToken<TNode>(TNode node, ErrorCode code, params object[] args) where TNode : XmlSyntaxNode
        {
            var firstToken = node.GetFirstToken();
            return WithAdditionalDiagnostics(node, MakeError(firstToken.GetLeadingTriviaWidth(), firstToken.Width, code, args));
        }

        protected TNode AddErrorToLastToken<TNode>(TNode node, ErrorCode code) where TNode : XmlSyntaxNode
        {
            int offset;
            int width;
        ...
        protected TNode AddErrorToLastToken<TNode>(TNode node, ErrorCode code, params object[] args) where TNode : XmlSyntaxNode
        {
            int offset;
            int width;
            GetOffsetAndWidthForLastToken(node, out offset, out width);
        ...
        private static void GetOffsetAndWidthForLastToken<TNode>(TNode node, out int offset, out int width) where TNode : XmlSyntaxNode
        {
        ...
        protected TNode AddLeadingSkippedSyntax<TNode>(TNode node, GreenNode skippedSyntax) where TNode : XmlSyntaxNode
        {
        ...
        protected TNode AddTrailingSkippedSyntax<TNode>(TNode node, GreenNode skippedSyntax) where TNode : XmlSyntaxNode
        {
        ...
        /// <returns></returns>
        private bool FindOffset(GreenNode root, XmlSyntaxNode location, out int offset)
        {
            int currentOffset = 0;
            offset = 0;
            if (root != null)
