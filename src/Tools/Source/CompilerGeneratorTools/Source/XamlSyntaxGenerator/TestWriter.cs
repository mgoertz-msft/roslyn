...
namespace XamlSyntaxGenerator
{
    internal class TestWriter : AbstractFileWriter
    {
        ...
        private void WriteFile()
        {
            ...
            WriteLine("namespace Microsoft.CodeAnalysis.Xaml.UnitTests");
            OpenBlock();
        ...
        private void WriteNodeGenerator(Node node, bool isGreen)
        {
            ...
            var xamlNamespace = isGreen ? "Microsoft.CodeAnalysis.Xaml.Syntax.InternalSyntax." : "";

            var strippedName = StripPost(node.Name, "Syntax");

            WriteLine("private static {0}{1} Generate{2}()", xamlNamespace, node.Name, strippedName);
            OpenBlock();

            //instantiate node
            {
                Write("return {0}SyntaxFactory.{1}(", xamlNamespace, strippedName);
                ...
                foreach (var field in nodeFields)
                {
                    ...
                    else if (IsAnyList(field.Type))
                    {
                        ...
                        if (isGreen)
                        {
                            typeName = internalNamespace + field.Type.Replace("<", "<" + xamlNamespace);
                        }
                        else
                        {
                    ...
                    else if (field.Type == "SyntaxToken")
                    {
                        ...
                        {
                            Write("{0}SyntaxFactory.Identifier(\"{1}\")", xamlNamespace, field.Name);
                        }
                        else if (kind == "StringLiteralToken")
                        {
                            Write("{0}SyntaxFactory.Literal({1}\"string\", \"string\"{2})", xamlNamespace, leadingTrivia, trailingTrivia);
                        }
                        else if (kind == "CharacterLiteralToken")
                        {
                            Write("{0}SyntaxFactory.Literal({1}\"a\", 'a'{2})", xamlNamespace, leadingTrivia, trailingTrivia);
                        }
                        else if (kind == "NumericLiteralToken")
                        {
                            Write("{0}SyntaxFactory.Literal({1}\"1\", 1{2})", xamlNamespace, leadingTrivia, trailingTrivia);
                        }
                        else
                        {
                            Write("{0}SyntaxFactory.Token(SyntaxKind.{1})", xamlNamespace, ChooseValidKind(field));
                        }
                    }
                    else if (field.Type == "XamlSyntaxNode")
                    {
                        Write("{0}SyntaxFactory.IdentifierName({0}SyntaxFactory.Identifier(\"{1}\"))", xamlNamespace, field.Name);
                    }
                    else
                    {
