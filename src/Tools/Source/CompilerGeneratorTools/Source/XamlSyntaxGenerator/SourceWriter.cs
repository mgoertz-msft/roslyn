...
namespace XamlSyntaxGenerator
{
    internal class SourceWriter : AbstractFileWriter
    {
        ...
        private void WriteInternal()
        {
            ...
            WriteLine("namespace Microsoft.CodeAnalysis.Xaml.Syntax.InternalSyntax");
            WriteLine("{");
            WriteLine();
        ...
        private void WriteSyntax()
        {
            ...
            WriteLine();
            WriteLine("namespace Microsoft.CodeAnalysis.Xaml.Syntax");
            WriteLine("{");
            WriteLine();
            this.WriteRedTypes();
            WriteLine("}");
        ...
        private void WriteMain()
        {
            ...
            WriteLine();
            WriteLine("namespace Microsoft.CodeAnalysis.Xaml");
            WriteLine("{");
            WriteLine("    using Microsoft.CodeAnalysis.Xaml.Syntax;");
            WriteLine();
        ...
        private void WriteGreenType(TreeType node)
        {
            ...
            else if (node is Node)
            {
                ...
                for (int i = 0, n = nodeFields.Count; i < n; i++)
                {
                    ...
                    {
                        WriteLine("    public {0}Microsoft.CodeAnalysis.Syntax.InternalSyntax.{1} {2} {{ get {{ return new Microsoft.CodeAnalysis.Syntax.InternalSyntax.{1}(new Microsoft.CodeAnalysis.Syntax.InternalSyntax.SyntaxList<XamlSyntaxNode>(this.{3})); }} }}",
                            OverrideOrNewModifier(field), field.Type, field.Name, CamelCase(field.Name), i
                            );
                    }
                    else if (field.Type == "SyntaxNodeOrTokenList")
                    {
                        WriteLine("    public {0}Microsoft.CodeAnalysis.Syntax.InternalSyntax.SyntaxList<XamlSyntaxNode> {1} {{ get {{ return new Microsoft.CodeAnalysis.Syntax.InternalSyntax.SyntaxList<XamlSyntaxNode>(this.{2}); }} }}",
                ...
                WriteLine("    {");
                WriteLine("      return new Xaml.Syntax.{0}(this, parent, position);", node.Name);
                WriteLine("    }");
        ...
        private void WriteGreenAcceptMethods(Node node)
        {
            ...
            //WriteLine("    }");
            WriteLine();
            WriteLine("    public override TResult Accept<TResult>(XamlSyntaxVisitor<TResult> visitor)");
            WriteLine("    {");
            WriteLine("        return visitor.Visit{0}(this);", StripPost(node.Name, "Syntax"));
            WriteLine("    }");
            WriteLine();
            WriteLine("    public override void Accept(XamlSyntaxVisitor visitor)");
            WriteLine("    {");
        ...
        private void WriteGreenVisitor(bool withArgument, bool withResult)
        {
            ...
            WriteLine();
            WriteLine("  internal partial class XamlSyntaxVisitor" + (withResult ? "<" + (withArgument ? "TArgument, " : "") + "TResult>" : ""));
            WriteLine("  {");
            int nWritten = 0;
            for (int i = 0, n = nodes.Count; i < n; i++)
            {
        ...
        private void WriteGreenUpdateMethod(Node node)
        {
            ...
            for (int f = 0; f < node.Fields.Count; f++)
            {
                ...
                var type =
                    field.Type == "SyntaxNodeOrTokenList" ? "Microsoft.CodeAnalysis.Syntax.InternalSyntax.SyntaxList<XamlSyntaxNode>" :
        ...
        private void WriteGreenRewriter()
        {
            ...
            WriteLine();
            WriteLine("  internal partial class XamlSyntaxRewriter : XamlSyntaxVisitor<XamlSyntaxNode>");
            WriteLine("  {");
            int nWritten = 0;
            for (int i = 0, n = nodes.Count; i < n; i++)
            {
                ...
                if (node != null)
                {
                    ...
                    if (nWritten > 0)
                        WriteLine();
                    nWritten++;
                    WriteLine("    public override XamlSyntaxNode Visit{0}({1} node)", StripPost(node.Name, "Syntax"), node.Name);
                    WriteLine("    {");
        ...
        private void WriteGreenFactory(Node nd, bool withSyntaxFactoryContext = false)
        {
                ...
                valueFields.Count + nodeFields.Count <= 3)
            {
                ...
                {
                    Write("      var cached = XamlSyntaxNodeCache.TryGetNode((int)");
                }
                else
                {
        ...
        private void WriteGreenFactoryParameters(Node nd)
        {
            ...
            for (int i = 0, n = nd.Fields.Count; i < n; i++)
            {
                ...
                {
                    type = "Microsoft.CodeAnalysis.Syntax.InternalSyntax.SyntaxList<XamlSyntaxNode>";
                }
                else if (IsSeparatedNodeList(field.Type) ||
                         IsNodeList(field.Type))
                {
        ...
        private void WriteRedType(TreeType node)
        {
            ...
            if (node is AbstractNode)
            {
                ...
                WriteLine("  {");
                WriteLine("    internal {0}(Microsoft.CodeAnalysis.Xaml.Syntax.InternalSyntax.XamlSyntaxNode green, SyntaxNode parent, int position)", node.Name);
            ...
            else if (node is Node)
            {
                ...
                for (int i = 0, n = nodeFields.Count; i < n; i++)
                {
                    ...
                    if (field.Type == "SyntaxToken")
                    {
                        ...
                        if (IsOptional(field))
                        {
                            WriteLine("        get");
                            WriteLine("        {");
                            WriteLine("            var slot = ((Microsoft.CodeAnalysis.Xaml.Syntax.InternalSyntax.{0})this.Green).{1};", node.Name, CamelCase(field.Name));
                        ...
                        {
                            WriteLine("      get {{ return new SyntaxToken(this, ((Microsoft.CodeAnalysis.Xaml.Syntax.InternalSyntax.{0})this.Green).{1}, {2}, {3}); }}", node.Name, CamelCase(field.Name), GetChildPosition(i), GetChildIndex(i));
                        }
                        WriteLine("    }");
                    }
                ...
                for (int i = 0, n = valueFields.Count; i < n; i++)
                {
                    var field = valueFields[i];
                    WriteComment(field.PropertyComment, "    ");
                    WriteLine("    {0} {1}{2} {3} {{ get {{ return ((Microsoft.CodeAnalysis.Xaml.Syntax.InternalSyntax.{4})this.Green).{3}; }} }}",
        ...
        private void WriteRedAcceptMethod(Node node, bool genericArgument, bool genericResult)
        {
            ...
            WriteLine();
            WriteLine("    public override " + (genericResult ? "TResult" : "void") + " Accept" + genericArgs + "(XamlSyntaxVisitor" + genericArgs + " visitor{0})", genericArgument ? ", TArgument argument" : "");
            WriteLine("    {");
        ...
        private void WriteRedVisitor(bool genericArgument, bool genericResult)
        {
            ...
            WriteLine();
            WriteLine("  public partial class XamlSyntaxVisitor" + genericArgs);
            WriteLine("  {");
            int nWritten = 0;
            for (int i = 0, n = nodes.Count; i < n; i++)
            {
        ...
        private void WriteRedRewriter()
        {
            ...
            WriteLine();
            WriteLine("  public partial class XamlSyntaxRewriter : XamlSyntaxVisitor<SyntaxNode>");
            WriteLine("  {");
            int nWritten = 0;
            for (int i = 0, n = nodes.Count; i < n; i++)
            {
        ...
        private void WriteRedFactory(Node nd)
        {
            ...
            Write("      return ({0})Microsoft.CodeAnalysis.Xaml.Syntax.InternalSyntax.SyntaxFactory.{1}(", nd.Name, StripPost(nd.Name, "Syntax"));
            if (nd.Kinds.Count > 1)
            {
                Write("kind, ");
            }
            for (int i = 0, n = nodeFields.Count; i < n; i++)
            {
                ...
                {
                    Write("{0}.Node.ToGreenList<Microsoft.CodeAnalysis.Xaml.Syntax.InternalSyntax.SyntaxToken>()", CamelCase(field.Name));
                }
                else if (IsNodeList(field.Type))
                {
                    Write("{0}.Node.ToGreenList<Microsoft.CodeAnalysis.Xaml.Syntax.InternalSyntax.{1}>()", CamelCase(field.Name), GetElementType(field.Type));
                }
                else if (IsSeparatedNodeList(field.Type))
                {
                    Write("{0}.Node.ToGreenSeparatedList<Microsoft.CodeAnalysis.Xaml.Syntax.InternalSyntax.{1}>()", CamelCase(field.Name), GetElementType(field.Type));
                }
                else if (field.Type == "SyntaxNodeOrTokenList")
                {
                    Write("{0}.Node.ToGreenList<Microsoft.CodeAnalysis.Xaml.Syntax.InternalSyntax.XamlSyntaxNode>()", CamelCase(field.Name));
                }
                else
                {
                    Write("{0} == null ? null : (Microsoft.CodeAnalysis.Xaml.Syntax.InternalSyntax.{1}){0}.Green", CamelCase(field.Name), field.Type);
                }
            }
