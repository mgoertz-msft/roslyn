...
namespace Microsoft.CodeAnalysis.Xaml
{
    internal partial class XamlSyntaxTreeFactoryServiceFactory
    {
        ...
        internal partial class XamlSyntaxTreeFactoryService
        {
            ...
            /// </summary>
            internal sealed class RecoverableSyntaxTree : XmlSyntaxTree, IRecoverableSyntaxTree<XmlBodySyntax>, ICachedObjectOwner
            {
                private readonly RecoverableSyntaxRoot<XmlBodySyntax> _recoverableRoot;
                ...
                private RecoverableSyntaxTree(AbstractSyntaxTreeFactoryService service, ProjectId cacheKey, XmlBodySyntax root, SyntaxTreeInfo info)
                {
                    _recoverableRoot = new RecoverableSyntaxRoot<XmlBodySyntax>(service, root, this);
                    _info = info;
                ...
                internal static SyntaxTree CreateRecoverableTree(AbstractSyntaxTreeFactoryService service, ProjectId cacheKey, string filePath, ParseOptions options, ValueSource<TextAndVersion> text, Encoding encoding, XmlBodySyntax root)
                {
                ...
                {
                    get { return (XmlParseOptions)_info.Options; }
                }
                ...
                private XmlBodySyntax CacheRootNode(XmlBodySyntax node)
                {
                ...
                public override XmlSyntaxNode GetRoot(CancellationToken cancellationToken = default(CancellationToken))
                {
                    return CacheRootNode(_recoverableRoot.GetValue(cancellationToken));
                }

                public override async Task<XmlSyntaxNode> GetRootAsync(CancellationToken cancellationToken)
                {
                ...
                XmlBodySyntax IRecoverableSyntaxTree<XmlBodySyntax>.CloneNodeAsRoot(XmlBodySyntax root)
                {
                ...
                public override SyntaxTree WithRootAndOptions(SyntaxNode root, ParseOptions options)
                {
                    ...
                    return Create((XmlSyntaxNode)root, this.Options, _info.FilePath);
                }
